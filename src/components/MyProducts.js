...
import { AcceptModal } from "./AcceptModal.js";
...
const MyProducts = ({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const validateSession_x = async () => {
        ...
        let res = await validateSession_call(process.env.BASE_UR, token);
        if (res === false) {
          setUser(null);
          return false;
        }
      ...
      if (tempSession === false) {
        ...
        let myLoads = await get_my_loads(
          base_url,
  ...
  const handleBidClick = (product, modal) => {
    ...
    if (modal === "accept_modal") setShowAcceptModal(true);
    else setShowFarmerModal(true);
  };
  ...
  return (
    <div id="my-products">
      ...
      >
        <button
          onClick={() => handleBidClick(null, "add_load_modal")}
          className="earth-500 hover:earth-700 text-white w-16 h-16 rounded-full shadow-xl flex items-center justify-center text-xl"
        >
